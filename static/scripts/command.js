...
var commands = [
    { name: "Generate new", icon: "refresh-cw",action: function() {RefreshPalette()}},
    { name: "Favorite palette", icon: "star", action: function() {FavPallete()}},
    { name: "Saved palettes", icon: "book", action: function() {OpenFav()}},
    { name: "Close", icon: "minimize-2", action: function() {closeMenu()}}
];
...
function FavPallete(){
    fetch(`/favorites?c1=${cCards[0].id.replace("#", "")}&c2=${cCards[1].id.replace("#", "")}&c3=${cCards[2].id.replace("#", "")}&c4=${cCards[3].id.replace("#", "")}&c5=${cCards[4].id.replace("#", "")}&prompt=null`, { method: "POST" })
...
commandSearch.addEventListener("input", function() {
    ...
    commandElements.forEach(function(commandElement) {
        var commandName = commandElement.querySelector('span').textContent.toLowerCase();

        if (commandName.includes(searchValue) || commandName.startsWith(searchValue)) {
            commandElement.style.display = "flex"
            visibleCommands++;
        } else {
